...
const createElements = (x) => {
    ...
    text.className= "text"
    text.innerHTML = `<h5>${x.original_title}</h5> <div class="card-panel" style="background:purple;color:white;"> ${(x.vote_average)} <span id=stars>${getStars(x.vote_average)}</span> </div>`
...
function getStars(rating) {
  ...
  for (var i = new_rating; i >= 1; i--)
    output.push('<i class="fa fa-star" aria-hidden="true" style="color: blue;"></i>&nbsp;');
...
const getMovies=() =>{
    fetch(`https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=69016672d7cf378f240778b1da922a04&page=${Math.floor(Math.random() * 100) + 1}`)
